...
import {
  isHelpChatbotOpenAtom,
  helpDynamicSuggestionsAtom,
  helpIsFetchingSuggestionsAtom,
} from '@/features/help-chatbot/atoms/helpChatbotAtoms';
import { HELP_SUGGESTED_QUESTIONS } from '@/features/help-chatbot/utils/systemPrompt';
...
async function fetchSuggestions(
  ...
  try {
    const response = await fetch('/api/help/chatbot/suggestions', {
      method: 'POST',
 ...
 * useHelpChatbot
 *
 * 一般利用者向けヘルプチャットボットの状態とアクションを管理するカスタムフック
 */
export function useHelpChatbot() {
  const [isOpen, setIsOpen] = useAtom(isHelpChatbotOpenAtom);
  ...
  const transport = useMemo(
    () => new DefaultChatTransport({ api: '/api/help/chatbot' }),
    []
  );
