...
export type NotificationBannerCloseProps = ComponentProps<'button'> & {
  label?: string;
};

const NotificationBannerClose = (props: NotificationBannerCloseProps) => {
  const { className, label, ...rest } = props;

  return (
    <button
      className={`
        rounded-6 -mr-3 inline-flex items-center gap-1 self-start px-3 pt-1
        pb-1.5 text-solid-gray-900
