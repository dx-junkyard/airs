...
import type { HourlyAnimalDataDto } from '@/server/application/dtos/ReportStatisticsDto';
import {
  getAnimalTypeColor,
  getAnimalTypeLabel,
  VALID_ANIMAL_TYPES,
...
interface HourlyAnimalChartProps {
  data: HourlyAnimalDataDto[];
  height?: number;
  ...
  if (active && payload && payload.length) {
    ...
    return (
      <div
        ...
        `}
      >
        <p className="mb-2 text-sm font-semibold text-solid-gray-800">{label}</p>
...
export default function HourlyAnimalChart({
  data,
  height = 300,
  containerClassName,
}: HourlyAnimalChartProps) {
  // データから動的な獣種キーを抽出（ドメイン定義順でソート）
  const animalKeys = useMemo(() => {
    const keySet = new Set<string>();
    data.forEach((d) => {
      Object.keys(d).forEach((k) => {
        if (k !== 'hour' && k !== 'label') keySet.add(k);
      });
    });
